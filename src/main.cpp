...
uint256 hashGenesisBlock("0xf96c0428a9d7c2f196fb4f0212a6b670abd3a55593bcbcd60d187e58f061b9fb");
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	else if(nHeight < 1181)
    {
        nSubsidy = 1 * COIN;
    }
    else if(nHeight < 1281)
    {
        nSubsidy = 2 * COIN;
    }
    else if(nHeight < 1381)
    {
        nSubsidy = 4 * COIN;
    }
	else if(nHeight < 1481)
    {
        nSubsidy = 8 * COIN;
    }
	else if(nHeight < 1781)
    {
        nSubsidy = 20 * COIN;
    }

    // Subsidy is cut in half every 2703085 blocks, which will occur approximately every 3 years
    nSubsidy >>= (nHeight / 2703085);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 35; // MasterCoin: 10 blocks
static const int64 nTargetSpacing = 35; // MasterCoin: 35 second blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal MasterCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MasterCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa1dfef3fc9dedff19181d2e81a739fc8d5435dab0f88a1010fbd12f97c8207fc");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "21 June 2013 07:00am AST";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1371943891; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1258157;

        if (fTestNet)
        {
            block.nTime    = 1371808829;
            block.nNonce   = 1220352;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x359dba21f3811dfeaef96a986781fa1243d35697679b543075fe7ce21a4c5157"));
